<div class="space-y-5">
                        ...
                        <tr class="hover:bg-gray-50 transition-colors">
                            ...
                            <td class="py-3.5 px-4 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    ...
                                    @can('delete', $genre)
                                        <button wire:click="confirmDelete({{ $genre->id }})"
    ...
    @if($isDeleteModalOpen && $deletingGenre)
        <div class="fixed inset-0 z-50 flex items-center justify-center p-4 overflow-y-auto" role="dialog" aria-modal="true">
            ...
            <div class="relative bg-white rounded-3xl border border-gray-100 shadow-2xl p-6 w-full my-auto z-10" style="max-width: 480px; width: 100%; margin-left: auto; margin-right: auto;">
                ...
                <div class="space-y-4">
                    <p class="text-sm text-gray-700">
                        Anda akan menghapus genre: <span class="font-extrabold text-gray-900">"{{ $deletingGenre->name }}"</span>
                    </p>
                    ...
                    @if($deletingBooksCount > 0)
                        <div class="p-4 rounded-2xl bg-amber-50 border border-amber-200 text-amber-900 space-y-2">
                            <div class="flex items-center gap-2 text-xs font-black uppercase tracking-wider text-amber-800">
                                <span>⚠️ Dibatalkan: Genre Masih Digunakan</span>
                            </div>
                            <p class="text-xs leading-relaxed">
                                Genre ini saat ini masih digunakan oleh <strong class="font-black text-amber-950 underline">{{ $deletingBooksCount }} buku</strong> di katalog perpustakaan.
                            </p>
                            <p class="text-[11px] text-amber-800 bg-amber-100/70 p-2.5 rounded-xl border border-amber-200 leading-normal">
                                <strong>Aturan Kebijakan Data:</strong> Untuk menjaga integritas data, Anda <u>tidak diizinkan menghapus</u> genre ini sampai seluruh buku yang terkait dipindahkan dari genre ini terlebih dahulu.
                            </p>
                        </div>
                    ...
                    @else
                        <div class="p-3.5 rounded-2xl bg-red-50 border border-red-200 text-red-800 text-xs leading-relaxed">
                            Genre ini <strong>tidak digunakan</strong> oleh buku manapun (0 buku). Apakah Anda yakin ingin menghapusnya? Tindakan ini tidak dapat dibatalkan.
                        </div>
                        ...
                        <div class="flex items-center justify-end gap-3 pt-3 border-t border-gray-100">
                            ...
                            <button wire:click="deleteGenre" class="px-4 py-2.5 rounded-xl bg-red-600 hover:bg-red-700 text-white font-bold text-xs shadow-sm transition-all active:scale-95">
                                Ya, Hapus Genre
                            </button>
                        </div>
                    @endif
