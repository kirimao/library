                    ...
                    <div class="space-y-3">
                        <div class="w-full aspect-[3/4] rounded-2xl bg-slate-100 border border-slate-200 overflow-hidden flex items-center justify-center text-slate-300 relative">
                            @if(!empty($b->cover_thumbnail))
                                <img src="{{ asset('storage/' . $b->cover_thumbnail) }}" alt="{{ $b->title }}" class="w-full h-full object-cover" onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';">
                                <div class="hidden w-full h-full items-center justify-center bg-slate-100 text-center p-4">
                                    ...
                                    <p class="text-[10px] font-bold text-slate-400 line-clamp-2">{{ $b->title }}</p>
                                </div>
                            @elseif(!empty($b->cover_image))
                                <img src="{{ asset('storage/' . $b->cover_image) }}" alt="{{ $b->title }}" class="w-full h-full object-cover" onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';">
                                <div class="hidden w-full h-full items-center justify-center bg-slate-100 text-center p-4">
                                    <svg class="w-12 h-12 mx-auto text-slate-300 mb-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253"/>
                                    </svg>
